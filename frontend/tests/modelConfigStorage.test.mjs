...
test("首次打开音频转文字配置时默认使用OpenAI Compatible", () => {
  assert.deepEqual(
...
test("已取消的音频识别方式回落到OpenAI Compatible默认配置", () => {
...
test("旧版单份Gemini配置不会填入OpenAI Compatible表单", () => {
...
test("Gemini与OpenAI Compatible配置分别保存且可分别恢复", () => {
